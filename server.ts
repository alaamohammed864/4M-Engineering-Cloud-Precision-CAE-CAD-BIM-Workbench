...
// Real geometry import (Priority 6): proxies the uploaded file to the
// Python/FastAPI backend, which parses it with the actual OpenCASCADE
// kernel (backend/app/geometry_kernel.py). No topology is fabricated here
// or in the backend - face/edge/vertex counts and volume come from OCCT's
// own algorithms. Requires authentication like the other mutating routes.
app.post('/api/geometry/import', auth.requireAuth, upload.single('file'), async (req: AuthedRequest, res) => {
  if (!req.file) {
    return res.status(400).json({ error: 'NO_FILE', message: 'No file was uploaded (expected multipart field "file")' });
  }
  try {
    const form = new FormData();
    form.append('file', new Blob([req.file.buffer]), req.file.originalname);
    const backendResponse = await fetch(`${FEA_BACKEND_URL}/geometry/import`, {
      method: 'POST',
      body: form,
    });
    const data = await backendResponse.json();
    ...
    res.status(502).json({
      error: 'GEOMETRY_BACKEND_UNREACHABLE',
      message: `Could not reach the geometry import backend at ${FEA_BACKEND_URL}. Original error: ${err.message}`,
    });
  }
});

// Proxies the real STL preview generated by the OpenCASCADE kernel so the
// Viewer3D component (Priority 3) can load actually-imported geometry,
// not just the bundled demo cube.
app.get('/api/geometry/:id/preview.stl', async (req, res) => {
  try {
    const backendResponse = await fetch(`${FEA_BACKEND_URL}/geometry/${req.params.id}/preview.stl`);
    if (!backendResponse.ok) {
      return res.status(backendResponse.status).json({ error: 'PREVIEW_NOT_FOUND' });
    }
    const buf = Buffer.from(await backendResponse.arrayBuffer());
    res.setHeader('Content-Type', 'model/stl');
    res.send(buf);
  } catch (err: any) {
    res.status(502).json({ error: 'GEOMETRY_BACKEND_UNREACHABLE', message: err.message });
  }
});
